...
// Modified query to only fetch orders for the current user using proses from pesanan table
$sql = "SELECT 
...
<script>
    ...
    <div class="container">
        <h1 class="text-2xl flex justify-center font-bold">My Orders</h1>
       ...
       <!-- Previous code remains the same until the table body -->
            <tbody id="orderTable">
                <?php
                if ($result->num_rows > 0) {
                    ...
                    while($row = $result->fetch_assoc()) {
                        ...
                        // Determine which process page to link to based on status
                        $process_page = "proses.html"; // default
                        if ($process_status == 2) {
                            $process_page = "proses_2.html";
                        } elseif ($process_status == 3) {
                            $process_page = "proses_3.html";
                        } elseif ($process_status == 4) {
                            $process_page = "proses_4.html";
                        }
                        
                        // Changed from pencil to eye icon and updated the href
                        echo "<td><a href='receipt.php?id=" . $row['transaction_id'] . "' class='edit-btn'>👁️</a></td>";
                        echo "</tr>";
                        $counter++;
                    }
                } else {
